...
export async function metricsRoutes(app: FastifyInstance, opts: MetricsRouteOptions): Promise<void> {
  app.get('/metrics/channels', { schema: { tags: ['metrics'], summary: 'Channel stats' } }, async (req) => {
  ...
  app.get('/metrics/raw/:id', { schema: { tags: ['metrics'], summary: 'Raw response' } }, async (req, reply) => {
